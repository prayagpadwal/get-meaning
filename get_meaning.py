...
# ---------------------------------------------------------------------------
# Single instance, auto-start, and crash logging
# ---------------------------------------------------------------------------
APP_NAME = "GetMeaning"
_LOCK_PORT = 47921  # arbitrary loopback port used as a single-instance lock


def _single_instance_lock():
    """Return a bound socket if this is the only instance, else None.

    The caller must keep the returned socket referenced for the whole process
    lifetime so the port stays held. A second launch can't bind it and knows
    to bow out — this is what prevents duplicate instances stacking up."""
...
def _pythonw():
    """Path to pythonw.exe (runs with no console); fall back to python.exe."""
...
def _autostart_file():
    return os.path.join(_startup_dir(), APP_NAME + ".vbs")


def _autostart_command(args):
    """Command line for autostart, preserving any non-default options."""
    parts = ['"%s"' % _pythonw(), '"%s"' % _script_path()]
    if args.hotkey != DEFAULT_HOTKEY:
        parts += ["--hotkey", '"%s"' % args.hotkey]
    if args.lang != DEFAULT_LANG:
        parts += ["--lang", args.lang]
    if float(args.timeout) != float(DEFAULT_TIMEOUT_S):
...
def install_autostart(args):
    if not IS_WIN:
        print("Auto-start is currently only implemented on Windows.")
        return 1
    startup = _startup_dir()
    if not os.path.isdir(startup):
        print(f"Couldn't find your Startup folder:\n  {startup}")
        return 1
    command = _autostart_command(args)
    literal = '"' + command.replace('"', '""') + '"'  # a VBScript string literal
    run_line = 'CreateObject("WScript.Shell").Run ' + literal + ", 0, False"
    vbs = "' Auto-generated by Get Meaning (--install-autostart)\r\n" + run_line + "\r\n"
    path = _autostart_file()
    try:
        with open(path, "w", encoding="utf-8") as fh:
            fh.write(vbs)
    except OSError as exc:
        print(f"Couldn't write the startup entry: {exc}")
        return 1
    print("Get Meaning will now start automatically when you log in.")
    print(f"  Startup entry: {path}")
    print("  It runs silently, with no console window.")
    print("Tip: double-click 'Start Get Meaning.vbs' to start it now without rebooting.")
    return 0
...
def uninstall_autostart():
    ...
    if not os.path.exists(path):
        print("Get Meaning wasn't set to start on login (nothing to remove).")
        return 0
    try:
        os.remove(path)
    except OSError as exc:
        print(f"Couldn't remove the startup entry: {exc}")
        return 1
    print("Removed Get Meaning from startup. It will no longer start on login.")
    return 0


def autostart_status():
    if not IS_WIN:
        print("Auto-start status is only tracked on Windows.")
        return 0
    path = _autostart_file()
...
class _NullStream:
    """A stdout/stderr stand-in for silent (pythonw) mode, where the real
    streams are None. Keeps print() from ever crashing the background app."""
...
def _log_error(exc):
    """Record a crash to a log file — important in silent (no-console) mode,
    where an exception would otherwise vanish without a trace."""
    try:
        logpath = os.path.join(os.path.dirname(_script_path()),
                               "get_meaning_error.log")
...
def parse_args(argv=None):
    ...
    parser.add_argument("--install-autostart", action="store_true",
                        help="Start Get Meaning automatically on login (Windows), "
                             "then exit.")
    parser.add_argument("--uninstall-autostart", action="store_true",
                        help="Remove Get Meaning from login startup, then exit.")
    parser.add_argument("--autostart-status", action="store_true",
                        help="Report whether auto-start is enabled, then exit.")
    return parser.parse_args(argv)


def main(argv=None):
    _silence_missing_streams()  # so print() is safe when launched via pythonw
    args = parse_args(argv)

    # One-shot management commands — do their thing and exit.
    ...
    # Refuse to run a second copy (keeps the hotkey from being double-bound).
    lock = _single_instance_lock()  # noqa: F841 — held for the process lifetime
    if lock is None:
        print("Get Meaning is already running.")
        return 0
